...
%     txSig=awgn(txSig,SNR,'measured');           % 此处不进行加噪(难以插值)
    ...
    for timeErr_index=1:length(timeErrVector)
        ...
        for freqErr_index=1:length(freqErrVector)
            parfor phaseErr_index=1:length(phaseErrVector)
                rxSig_checked=TEmodel(txSig,timeErrVector(timeErr_index));
                rxSig_checked=awgn(rxSig_checked,SNR,'measured');
                ...
                rxSig_checked=rxfilter(rxSig_checked);
                rxQPSK_symbol_stream=rxSig_checked((span*sps)+1:end);
                rxQPSK_symbol_stream=rxQPSK_symbol_stream(1:sps:end);
                ...
                Object_Fun_1(phaseErr_index,freqErr_index,timeErr_index)=0.5*sum(log(1+cosh(LLR_vector_self)));
                %% 校验子范数
...
%% 绘图输出
% figure(10)
% plot(timeErrVector,reshape(Object_Fun_1(1,1,:),[],1))
% 
% [X,Y]=meshgrid(phaseErrVector,freqErrVector);
% figure(1)
% mesh(X,Y,Object_Fun_1(:,:,3).')
% xlim([-180 180])
% xticks(-180:90:180)
% xlabel('载波相位 (deg)')
% ylabel('载波频率 (NFO)')
