%% 主执行脚本：该系统采用QPSK调制，分析载波中相位-频率误差对目标函数的影响
clc
clear
close all
rng default
...
%% 测试控制参数
phaseErrVector=-180:10:180;
freqErrVector=(-8:2:8)*1e-4;
EbN0_scale=4;                                   % EbN0测试范围  (译码门限 2)
BP_times=1;                                     % BP译码迭代次数
nFrame=1;                                       % 单次测试帧数
...
rxfilter = comm.RaisedCosineReceiveFilter('RolloffFactor',rolloff,'FilterSpanInSymbols',span,'DecimationFactor',sps,'InputSamplesPerSymbol',sps);        % 收端根升余弦滚降滤波器
...
for i=1:length(EbN0_scale)
    ...
    %% 收端测试
    Object_Fun_1=zeros(length(phaseErrVector),length(freqErrVector));       % LLR目标函数
    Object_Fun_2=zeros(length(phaseErrVector),length(freqErrVector));       % 校验子范数
    
    for freqErr_index=1:length(freqErrVector)
        disp([num2str(freqErr_index) '/' num2str(length(freqErrVector))])
        col_1=zeros(length(phaseErrVector),1);
        col_2=zeros(length(phaseErrVector),1);
        for phaseErr_index=1:length(phaseErrVector)
            ...
            rxSig_checked=freqErrIterator(rxSig);
            ...
            col_1(phaseErr_index)=0.5*sum(log(1+cosh(LLR_vector_self)));
            
            %% 校验子范数
             Hard_vector=(LLR_vector_self<=0);   
             col_2(phaseErr_index)=sum(1-double(rem(sum(H(:,Hard_vector),2),2)));
        end
        Object_Fun_1(:,freqErr_index)=col_1;
        Object_Fun_2(:,freqErr_index)=col_2;
    end
