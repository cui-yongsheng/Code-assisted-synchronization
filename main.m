...
timingErr=1;                              % 附加时延
% (样本点数)
EbN0_scale=2;                           % EbN0测试范围(理想同步时，码率1/2译码门限2dB) Todo:译码门限计算
nFrame=1000;                               % 测试帧数
...
txfilter = comm.RaisedCosineTransmitFilter( ...
    ...
    'OutputSamplesPerSymbol',sps);                              % 发端-根升余弦滚降滤波器
freqOffset = comm.PhaseFrequencyOffset...
    ('FrequencyOffset',FreqErr,....
    'PhaseOffset',PhaseErr,'SampleRate',sps);                     % 发端-附加频偏、相位模块
...
ldpcDec = comm.LDPCDecoder('ParityCheckMatrix',H, ...
    ...
    'MaximumIterationCount',20,'OutputValue','Whole codeword'); % 收端-LDPC解码器
...
for i=1:length(EbN0_scale)
    ...
    %% 引入信道噪声   参考:《数字通信系统中Eb/N0与SNR转换方法的研究》
    ...
    rxSig=awgn(txSig,SNR,'measured');                           % 添加白噪声
    ...
    tic
    disp('收端测设开始...');
    parfor frame_id=1:nFrame
        if(mod(frame_id,100)==0)
            disp(['####### 第 ' num2str(frame_id) ' 帧  #######']);
        end
        rxFrame_checked_sample=rxQPSK_symbol_stream(:,frame_id);
        %% 相频时延预搜索
        [phasePre,freqPre,timePre]=phaseFreqTimeSearch(rxFrame_checked_sample,symbol_noise_var,H,sps);
        %% 使用预搜索结果修正
        Error_pre_Iterator=comm.PhaseFrequencyOffset('PhaseOffset',phasePre,'FrequencyOffset',freqPre,'SampleRate',sps);
        rxFrame_checked_sample=Error_pre_Iterator(rxFrame_checked_sample);
        rxFrame_checked_sample=TEmodel(rxFrame_checked_sample,timePre);
        %% DNN预测
        [Est_phase_d, Est_freq_d,Est_time_d]=single_DNN_estimate_joint(rxFrame_checked_sample,symbol_noise_var,H,sps);
        %% 使用预测结果修正
        Error_amend_Iterator=comm.PhaseFrequencyOffset('PhaseOffset',-Est_phase_d(end),'FrequencyOffset',-Est_freq_d(end),'SampleRate',sps);
        rxFrame_checked_sample=Error_amend_Iterator(rxFrame_checked_sample);
        rxFrame_checked_sample=TEmodel(rxFrame_checked_sample,-Est_time_d(end));
        %% EM预测
        [Est_phase_v,Est_freq_v,Est_time_v,LLR_vector]=sequential_EM_estimate_joint(rxFrame_checked_sample,symbol_noise_var,H,sps);
        %% 使用LLR译码
    ...
    toc
    BER_test(i)=mean(BER);                  % 误码率统计
    disp(['收端测设结束,误码率为:' num2str(BER_test(i))]);
